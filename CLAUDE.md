...
`src/services/authService.ts` and `src/services/scanService.ts` define interfaces (`AuthService`, `ScanService`) with `Mock*` implementations exported as singletons (`authService`, `scanService`). Screens and contexts only ever import the interface-typed singleton, never the `Mock*` class directly — swapping in a real backend later means writing a new class that implements the same interface, no call-site changes. `MockAuthService` keeps accounts in memory keyed by (lowercased) email — signup rejects an email that's already registered, login checks the matching account's password. On web, accounts are mirrored to `localStorage` (shared across tabs, so an account created in one tab is visible in another) but the logged-in session itself is mirrored to `sessionStorage` instead (see `authService.ts`/`AuthContext.tsx`) — this is deliberate: it's per-tab, so two tabs can be signed into two different accounts at once without one clobbering the other's session, at the cost of a brand-new tab starting signed out rather than auto-continuing an existing session. There's still no real backend, no password hashing, and native (iOS) has no persistence across app restarts since it has no `localStorage`/`sessionStorage` equivalent wired up.
